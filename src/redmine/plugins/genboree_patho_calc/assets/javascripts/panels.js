...
function getReasonerData(result, request, reqObj)
{
  ...
  var reasOut = result.data ;

  // update the reasoner output with the grid and cache
  //var gName = result.request.options.params.gridname ;
  var gName ;
  var paramOpts = this.url.split("&") ;
  for(var ii=0; ii<paramOpts.length; ii++){
    var po = paramOpts[ii] ;
    if(po.match(/gridname/)){
      gName = po.split('=')[1] ;
    }
  }
...
function addTabs()
{
  ...
  else if(grids[firstTab].hasnotags)
  {
    ...
    firstItems.push(getPhenoAndMOITable()) ;
    ...
    firstItems.push(getPhenoAndMOITable()) ;
    firstItems.push(evTable);
    firstItems.push(getTagsLabelTable()) ;
   }
   else // no evidence
   {
     ...
     listeners:{
        activate: function(tab){
            ...
            if(grids[tabName].hasnotags)
            {
             ...
             tab.add([hasnomesGrid, getPhenoAndMOITable(), evTable]);
            }
            else if(grids[tabName].evidence)
            {
              ...
              if(grids[tabName].guideline)
              {
                ...
                tab.add([reasGrid, getPhenoAndMOITable(), evTable, getTagsLabelTable()]) ;
              }
              else
              {
                noguidelineState(tabName) ;
                tab.add([noguidesgrid, getPhenoAndMOITable(), evTable, getTagsLabelTable()]) ;
              }
  ...
  if(grids[firstTab].evidence)
  {
    gridtab.setActiveTab(tab) ;
    activeyet = true ;
    updatePhenoAndMOITable(grids[firstTab].evidence) ;
  }
 ...
 if(Ext.getCmp('tabMask')){ Ext.getCmp('tabMask').destroy() ;}
  ...
  for(var ii=0; ii<gridNames.length; ii++)
  {
    ...
    if(grids[gridNames[ii]].evidence)
    {
      ...
      secondRow[safeStr[gridNames[ii]]] = '<div id="tabicon" align="center"><span id="'+safeStr[gridNames[ii]]+'" class="tohideTab" onclick="toggletab(\'' +gridNames[ii]+'\')">HIDE</span></br></div>' ;
    ...
    else // no evidence
    {
      ...
      secondRow[safeStr[gridNames[ii]]] = '<div id="tabicon" align="center"><span id="'+safeStr[gridNames[ii]]+'" class="tocreateTab" onclick="toggletab(\'' +gridNames[ii]+'\')">CREATE</span></br></div>' ;
