    ...
    items: [
      {
        ...
        listeners: {
          beforequery: function(queryEvent, eOpts) {
            var saPath = decodeURIComponent(regAddress) ;
            if(queryEvent.combo.rawValue.match(/^\s*$/))// if empty do a default search just on the coll with a limit
              {
                saPath = saPath + '/alleles?namePrefix=NC&limit=25' ;
              }
              else
              {
                saPath = saPath + '/alleles?namePrefix='+encodeURIComponent(queryEvent.combo.rawValue)+'&limit=25' ;
              }
...
function makeQuery()
{
  ...
  {
     var regUrl = getAppLink() ;
     regUrl = regUrl+'?reg=true&hgvs='+encodeURIComponent(formVals.hgv) ;
