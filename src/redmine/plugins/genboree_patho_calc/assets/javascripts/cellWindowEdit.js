...
function showAddLinksWindow(selectedTag, subj, parPath, cellId, selectedTagcode) 
{
  ...
  if((subj && subj !== "") && ('Links' in subj.Tag.properties) && ('items' in subj.Tag.properties.Links) && (subj.Tag.properties.Links.items.length > 0))
  {
...
function makeLinksWindow(linkStoreData, parPath, selectedTag, selectedTagcode) 
{
  ...
  var lcodeStore = Ext.create('Ext.data.Store', {
    fields: ['lcode'],
    data: [{"lcode": "Supports"}, {"lcode": "Unknown"}, {"lcode": "Disputes"}]
  }) ;
    ...
    [
      ...
      },
      {
        ...
        editor: {
          ...
          regexText: "This is not a valid URL. URLs must start with http://, https://, ftp://, www, etc",
      ...
      },
      {
        ...
        listeners: {
          afterender: function(){
            //debugger ;
            //if(Ext.getCmp("linkcodeCombo").getValue() === ""){
            //  Ext.getCmp("linkcodeCombo").setValue("Supports") ;
            //}
          }
        },
        editor: {
          ...
          forceSelection : true,
          //valueField: 'lcode',
          //store: [ { val: 'Unknown' }, { val: 'Supports' }, { val: 'Disputes' }],
          //store:  lcodeStore,
          store: ['Supports', 'Unknown', 'Disputes'],
          validator: function(fieldValue) {
            var retVal = true ;
            // Did they select an actual tag or the generic "CHOOSE ONE" enum val from the model? Let's encourage the proper tag/code here.
            var grid = linkGrid ;
