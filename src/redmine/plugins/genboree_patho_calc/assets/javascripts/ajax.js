...
Ext.define('HgVs', {
     extend: 'Ext.data.Model',
       proxy: {
         type: 'ajax',
         //url : '/java-bin/apiCaller.jsp',
         //url: decodeURIComponent(regAddress),
         url: '',
         timeout : 90000,
         noCache: false,
         cors: true,
...
{
  getAlleleFromReg(hgvsTerm, true) ;
}

function getAlleleFromReg(parTerm, useHgvs)
{
  var resourcepath;
  var qparams = {'term': parTerm} ;
  if(useHgvs)
  {
    resourcepath = '/allele?hgvs='+encodeURIComponent(parTerm) ;
    qparams.hgvs = 'true' ;
  }
  else
  {
    resourcepath = '/allele/'+encodeURIComponent(parTerm) ;
  }
  var allelePath = decodeURIComponent(regAddress) ;
  var simpleSubjects = [];
  $.ajax(appPath+"/allele_registry/find_by_id", {
    data : qparams,
    method : "GET",
    success: function(apiRespObj){
      apiRespObj = JSON.parse(apiRespObj) ;
      if(!('@id' in apiRespObj))
      {
    ...
    error: function(respObj){
      if(Ext.getCmp('entryMask')){
        Ext.getCmp('entryMask').destroy() ;
      }
      if(useHgvs){ 
        initAlleleRegistry(parTerm) ;
      }
      else{
        Ext.Msg.alert("NOT FOUND", "The allele with the id "+parTerm+" could not be found.") ;
      }
    }
  }) ;
...
function viewGrid(format, gridName)
{
  ...
  {
    //url : '/java-bin/apiCaller.jsp' ,
    url: appPath+"/evidences/transform",
    timeout : 180000,
    method: 'GET',
    params:
    {
...
function runReasonerGrid(gridn)
{
  ...
  $.ajax(appPath+'/reasoner/run', {
    method: 'GET',
    data: params,
    dataType: 'json',
    success: getReasonerData,
    error: displayFailureDialogReasoner
  }) ;
...
function saveEviDoc(evDocument, changeCls, cellId, grname, hasnotagsinfo, closeWindow, refreshTableOnly)
{
  ...
  var moi = evDocument['Allele evidence']['properties']['Subject']['properties']["Mode of inheritance"]["value"] ;
  var pheno = evDocument['Allele evidence']['properties']['Subject']['properties']["Phenotype"]["value"] ;
...
function updateCache(gname)
{
  ...
  Ext.Ajax.request(
  {
    ...
    params:
    {
      ...
      apiMethod : 'GET',
      grName : gname,
      matchValues: grids[gname].guideline+','+evidenceUrl
    },
    method: 'GET',
    success: updateData,
...
function updateData(result, request)
{
  ...
  $.ajax(appPath+"/conclusion_cache/update?"+csrf_param+'='+escape(csrf_token), {
    data: params,
    method: "POST",
  }) ;
...
function getTagStore(gn, format, makeEmptyGrid)
{
  ...
  {
    //url : '/java-bin/apiCaller.jsp' ,
    url: appPath+'/source_registry/tags',
    timeout : 180000,
    method: 'GET',
    params:
    {
      rsrcPath: allowedTagPath,
      evSourceUrl: decodeURIComponent(evSourceUrl),
      apiMethod : 'GET',
      gName : gn,
    },
    callback: function(opts, success, response)
...
function getCAsFromAllEvidenceSources()
{
  ...
  {
    //url : '/java-bin/apiCaller.jsp' ,
    url: kbMount+"/projects/"+projectId+"/genboree_patho_calc/source_registry/all",
    timeout : 90000,
    method: 'GET',
    params:
    {
      ...
      if(response.status >= 200 && response.status < 400 && apiRespObj)
      {
        ...
        for (var ii=0; ii<sourceRegistry.length; ii++)
        {
          ...
          for(var jj=0; jj<evisourceItems.length; jj++)
          {
            ...
            {
              //url : '/java-bin/apiCaller.jsp' ,
              url:  kbMount+"/projects/"+projectId+"/genboree_patho_calc/evidences/all",
              timeout : 90000,
              method: 'GET',
              params:
              {
...
function getSourceRegistry(makeGrid)
{
  ...
  {
    //url : '/java-bin/apiCaller.jsp' ,
    url: appPath+"/source_registry/all",
    timeout : 90000,
    params:
    {
      //rsrcPath: sourceRegPath,
...
function getConclusionForGrids(gridName)
{
 ...
 { 
    ...
    cachePath = cachePath+'/docs?detailed=true&matchProps=ConclusionCacheID.Guideline,ConclusionCacheID.Evidence%20Doc&matchValues='+grids[gridName].guideline+','+evidenceDocUrl+'&matchLogicOp=and' ;
    Ext.Ajax.request(
    {
      //url : '/java-bin/apiCaller.jsp' ,
      url: appPath+'/conclusion_cache',
      ...
      params:
      {
        ...
        apiMethod : 'GET',
        gridname: gridName,
        matchValues: grids[gridName].guideline+','+evidenceDocUrl
      },
      callback: function(opts, success, response)
      {
  ...
  {
    if(grids[gridName].evidence){
      grids[gridName].conclusion = 'Undetermined' ;
    } //  no guidelines
    else {
      grids[gridName].conclusion = "" ;
    }
    if(grids[gridName].last){
      setTimeout(addTabs, 2000);
    }
  }
}
...
function updateEvidenceCache(gridn)
{
  ...
  {
    //url : '/java-bin/apiCaller.jsp' ,
    url: appPath+'/ca_evidence_caches/find_by_id',
    timeout : 180000,
    method: 'GET',
    params:
    {
...
function saveEvidenceCache(cacheDoc)
{
  ...
  //params[csrf_param] = csrf_token ;
  $.ajax(appPath+'/ca_evidence_caches/save?'+csrf_param+'='+escape(csrf_token),{
      //url : '/java-bin/apiCaller.jsp' ,
    //url: 
    timeout : 90000,
    //beforeSend: function(xhr) {xhr.setRequestHeader('X-CSRF-Token', $('meta[name="csrf-token"]').attr('content'))},
  ...
  // update the Assertion in the summary and evidence doc in the evidence doc
  params = {} ;
  params[csrf_param] = csrf_token ;
  params.assertion = cacheDoc.EvidenceCacheID.properties.FinalCall.value ;
  params.type = cacheDoc.EvidenceCacheID.properties.Type.value ;
  $.ajax(appPath+"/allele/"+ca+"/assertion-summary/update?", {
    data: params,
    method: "POST",
    success: function (){
      params = {} ;
      ...
      params.assertion = cacheDoc.EvidenceCacheID.properties.FinalCall.value ;
      params.type = cacheDoc.EvidenceCacheID.properties.Type.value ;
      params.gridDisplayName = Ext.getCmp('multitab').getActiveTab().title ;
      $.ajax(appPath+"/allele/"+ca+"/evidence/assertion/update?", {
        method: 'POST',
        data: params
      }) ;
    }
...
function getalleleInfoFomReg(ca, taskId, entryInnerId)
{
  ...
  var allelePath = appPath+"/allele_registry/find_by_id?term="+ca ;
  //allelePath = window.location.protocol+"//"+allelePath + '/allele/'+ ca+"?" ;
  $.ajax(allelePath, {
    data : {},
    method : "GET",
    timeout: 10000,
    success : function(respObj) {
      ...
      topPanelInfo[ca].hgvsWithProteinVariation = [] ;
    ...
    error : function(respObj) {
      ...
      else
      {
        ...
        displayMsg += ( "<b>Error Code:</b> <i>" + (respObj['status'] ? respObj['status'] : "[ NOT INTELLIGIBLE ]") + "</i><br>" ) ;
        displayMsg += ( "<b>Error Message:</b> <i>" + (respObj['statusText'] ? respObj['statusText'] : "[ NOT INTELLIGIBLE ]") + "</i><br>" );
        displayMsg += "<br><br>Please contact a project admin to resolve this issue." ;
        Ext.Msg.alert("ERROR", displayMsg) ;  
      }  
    }
...
function getGeneFromRegistry(ca, taskId, entryInnerId)
{
  ...
  if(Object.keys(topPanelInfo[ca].geneSubject).length > 0)
  {
    ...
    var genePathEls = genePath.split("/") ;
    var gene = genePathEls[genePathEls.length-1] ;
    $.ajax(appPath+"/allele_registry/find_by_gene", {
      data : {gene: gene},
      dataType: 'json',
      method : "GET",
      timeout: 10000,
    ...
    getCASourceLinks() ; trackData(taskId, 0, entryInnerId); /* see helper.js */
  }
}
...
function getEntryStore()
{
  ...
  var entStore = new Ext.data.Store({
    ...
    proxy: {
      type: 'ajax',
      //url: '/java-bin/apiCaller.jsp',
      //url: urlMount+'/ca_evidence_caches',
      url: kbMount+"/projects/"+projectId+"/genboree_patho_calc/ca_evidence_caches",
      actionMethods: {
        read: 'GET'
      },
      reader: {
    ...
    listeners: {
      beforeload: function(store, operation, options){
        ...
        //store.proxy.extraParams.rsrcPath = escape(evCachePath);
        store.proxy.extraParams.casubjectList = JSON.stringify(casubjectList) ;
        store.proxy.extraParams[csrf_param] = csrf_token ;
