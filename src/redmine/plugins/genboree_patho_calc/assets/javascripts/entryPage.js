...
function getsearchMenu()
{
  ...
  var searchMenu = new Ext.menu.Menu({
    ...
    items: [
    {
      text: 'Search with HGVS expression',
      iconCls: "gvAdvanced",
      tooltip: 'Search Allele Registry using HGVS term.',
      handler : function(){ getAdvancedSearchWin() ;}
    },
    {
      text: 'Search with CA identifier',
      iconCls: 'gvAdvanced',
      tooltip: 'Search Allele Registry using canonical allele identifier',
      handler :  getCAidWindow 
    },
    {
      text: 'Search with Gene',
      iconCls: 'gvAdvanced',
...
function getEntryToolbar(searchMenu)
{
  var toolBar = new Ext.Toolbar({
    ...
    {
      text: 'Search Variant to Infer Pathogenicity',
      id: "gvAdvanced",
      iconCls: 'gvAdvanced',
      menu: searchMenu,
...
{
  var url = window.location.protocol+"//"+decodeURIComponent(regAddress)+"/allele/"+allele+".html" ;
  window.open(url, "alleleRegistry") ;
}
