// Generate an empty grid for html and small html format
// If tags is not present it gets the tags data from the allowed tags
// The invalid cells are identified from the allowed tags
// tags is global here
function getEmptyGridFromTag(tags, format, evDoc)
{
  if(isEmpty(tags))
  {
    getTagStore(true, format, evDoc) ;
  }

  else
  {
    makeEmptyTable(tags, format, evDoc) ;
  }
...
function makeEmptyTable(allowedTags, format, evDoc)
{
  ...
  else if(format == 'html')
  {
    ...
    for(var ii=0; ii<partitions[2].length; ii++)
    {
      ...
      emptyTable += '<th id='+id+' class="gb-part3-header gb-part3-header-'+(ii+1)+'" onclick="clickPartition(\''+id+'\', \''+partitions[2][ii]+'\', \''+partitions[2][ii]+'\', null)" >' +partitions[2][ii]+'</th>' ;
      for(var jj=0; jj<partitions[0].length; jj++)
      {
        for(var kk=0; kk<partitions[1][jj].length; kk++)
        {
